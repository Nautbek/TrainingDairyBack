...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | Перевод стандартных сообщений валидатора Laravel на русский язык.
    | Набор ключей — точная копия lang/en/validation.php (Laravel 12), чтобы
    | ничего не выпало по фолбэку на английский.
    |
    */
    ...
    'password' => [
        ...
        'uncompromised' => 'Указанный :attribute был скомпрометирован в результате утечки данных. Пожалуйста, выберите другой :attribute.',
    ],
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Человекочитаемые названия полей для :attribute — без этого в тексте
    | ошибки было бы "Поле email ..." вместо "Поле «адрес электронной почты» ...".
    | Список ограничен полями, реально встречающимися в форм-реквестах проекта
    | (email-аутентификация плюс несколько повсеместных общих полей).
    |
    */
