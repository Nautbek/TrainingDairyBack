...
namespace Modules\MyCar\Services;
...
class MyCarPaymentService implements PaymentAppHandler
{
    ...
    {
        return $payment->app === (string) config('mycar.app');
    }
    ...
    private function notifyPaymentSucceeded(
        ...
        $message = "My Car оплата: {$payment->amount} ₽, {$payment->months} мес.\n"
            ."Пользователь: {$payment->user_uuid}\n"
            ."Способ: {$method}";

        if (is_string($yookassaPaymentId) && $yookassaPaymentId !== '') {
