...
namespace Modules\TripSplit\Tests\Feature;

use App\Enums\Donation\PaymentStatus;
use App\Models\DonationPayment;
use Modules\TripSplit\Models\UsageBalance;
use App\Models\User;
...
class TripSplitPaymentTest extends TestCase
{
        ...
        $this->instance(
            ...
            Mockery::mock(TelegramNotificationService::class, function ($mock) use ($uuid): void {
                $mock->shouldReceive('sendMessage')
                    ->once()
                    ->withArgs(function (string $message) use ($uuid): bool {
                        return str_contains($message, 'TripSplit оплата: 99 ₽, 1 подсчёт(ов)')
                            && str_contains($message, "Пользователь: {$uuid}")
                            && str_contains($message, 'Способ: bank_card')
                            && str_contains($message, 'ЮKassa: yk-tripsplit-payment-id')
                            && str_contains($message, 'Баланс: 1');
                    })
                    ->andReturn(true);
            }),
        );
    ...
    public function test_consume_credits_decrements_balance(): void
    {
        ...
        $service = app(\Modules\TripSplit\Services\TripSplitCreditsService::class);
